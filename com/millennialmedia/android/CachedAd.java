  ...
  implements Externalizable
{
  ...
  static CachedAd parseJSON(String paramString)
  {
    if (MMSDK.logLevel >= 5)
    {
      ...
      while (k < j)
      {
        MMLog.v("CachedAd", paramString.substring(m, k));
        m = k;
        k += 1000;
        if (k > j) {
          k = j - 1;
  ...
  {
    File localFile = AdCache.getInternalCacheDirectory(paramContext);
    if ((localFile != null) && (localFile.isDirectory())) {
      try
      {
        ...
        MMLog.v("CachedAd", String.format("Deleting %d files for %s.", arrayOfObject));
      ...
      {
        MMLog.e("CachedAd", "Exception deleting cached ad: ", localException);
      }
    }
  }
  ...
  protected void deserializeFromObj(JSONObject paramJSONObject)
  {
    ...
    {
      MMLog.e("CachedAd", "Exception deserializing cached ad: ", localParseException);
    }
  }
